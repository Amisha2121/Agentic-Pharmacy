        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-8 shadow-xl border-2 border-[#1E4A4C]/10">
          <div className="flex items-center gap-6 mb-8 pb-8 border-b border-gray-200">
            ...
            <div>
              ...
              {user?.email && (
                <p className="text-gray-400 text-sm mt-0.5 font-mono">{user.email}</p>
              )}
              <div className="flex items-center gap-2 mt-1.5">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full capitalize ${
                  user?.provider === 'google' ? 'bg-blue-50 text-blue-600' :
                  user?.provider === 'phone'  ? 'bg-violet-50 text-violet-600' :
                  user?.provider === 'email'  ? 'bg-emerald-50 text-emerald-600' :
                ...
                <span className="text-gray-400 text-xs font-medium">Staff Access</span>
              </div>
            </div>
          </div>
