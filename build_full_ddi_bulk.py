...
# Try using cached file first to avoid kagglehub hanging on extraction
CACHED_CSV = Path(
    r"C:\Users\AMISHA\.cache\kagglehub\datasets"
    r"\singhnavjot2062001\11000-medicine-details\versions\87\Medicine_Details.csv"
)

medicine_df = None
if CACHED_CSV.exists():
    print(f"  Using cached file: {CACHED_CSV}")
    try:
        medicine_df = pd.read_csv(CACHED_CSV)
        print(f"  Loaded Medicine_Details.csv: {medicine_df.shape[0]} rows")
    except Exception as e:
        print(f"  Failed to read cached CSV: {e}")
