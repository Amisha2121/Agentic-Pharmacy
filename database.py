...
def get_inventory():
    ...
    try:
        docs = db.collection(COLLECTION).stream(timeout=10)
        results = []
        for doc in docs:
            d = doc.to_dict()
            stock_val = 0
...
def list_chat_sessions(limit: int = 30) -> list[dict]:
    ...
    try:
        docs = db.collection(CHAT_SESSIONS_COLLECTION).order_by("updated_at", direction=firestore.Query.DESCENDING).limit(limit).get(timeout=5)
...
def get_todays_sales_log() -> list[dict]:
    ...
    docs = db.collection(SALES_LOG_COLLECTION).where(filter=firestore.FieldFilter("date", "==", today_str)).stream()
    results = []
    for doc in docs:
        d = doc.to_dict()
        d["log_id"] = doc.id
...
def get_sales_history(days: int = 5) -> dict[str, list[dict]]:
    ...
    for offset in range(1, days + 1):
        ...
        docs = db.collection(SALES_LOG_COLLECTION).where(filter=firestore.FieldFilter("date", "==", day_str)).stream()
        entries = [ {**doc.to_dict(), "log_id": doc.id} for doc in docs ]
        if not entries:
            arch_docs = db.collection("archived_sales_log").where(filter=firestore.FieldFilter("date", "==", day_str)).stream()
            entries = [ {**doc.to_dict(), "log_id": doc.id} for doc in arch_docs ]
        result[day_str] = entries
    return result

def get_archived_sales(page: int = 0, page_size: int = 10) -> tuple[dict[str, list[dict]], int]:
    """Fetch paginated archived sales logs."""
    if MOCK_MODE:
        return {}, 0
    cutoff = (datetime.date.today() - datetime.timedelta(days=5)).isoformat()
    all_docs = list(db.collection("archived_sales_log").where(filter=firestore.FieldFilter("date", "<", cutoff)).stream())
    grouped = {}
    for doc in all_docs:
        d = doc.to_dict()
...
def update_sales_log_entry(log_id: str, new_qty: int):
    ...
    if doc.exists:
        ...
        if d.get("stock_deducted", False) and batch_number:
            _adjust_stock_by_batch(batch_number, old_qty - new_qty)
...
def delete_sales_log_entry(log_id: str):
    ...
    if doc.exists:
        ...
        if d.get("stock_deducted", False) and d.get("batch_number"):
            _adjust_stock_by_batch(d["batch_number"], int(d.get("qty_sold", 0)))
        doc_ref.delete()
...
def process_midnight_deductions():
    ...
    today_str = datetime.date.today().isoformat()
    try:
        past_docs = list(db.collection(SALES_LOG_COLLECTION).where(filter=firestore.FieldFilter("date", "<", today_str)).get(timeout=10))
    except Exception as e:
    ...
    for doc in past_docs:
        ...
        if not d.get("stock_deducted", False) and d.get("qty_sold", 0) > 0 and d.get("batch_number"):
            _adjust_stock_by_batch(d["batch_number"], -int(d["qty_sold"]))
            _reset_reorder_flag_if_zero(d["batch_number"])
...
def get_analytics_data() -> dict:
    ...
    if MOCK_MODE:
        return {"sales": [], "stock": get_inventory_with_stock()}
    today = datetime.date.today()
    thirty_days_ago = (today - datetime.timedelta(days=30)).isoformat()
    sales_docs = list(db.collection(SALES_LOG_COLLECTION).where(filter=firestore.FieldFilter("date", ">=", thirty_days_ago)).stream())
    archived_docs = list(db.collection("archived_sales_log").where(filter=firestore.FieldFilter("date", ">=", thirty_days_ago)).stream())
    return {
        "sales": [d.to_dict() for d in sales_docs + archived_docs],
        "stock": get_inventory_with_stock()
    }